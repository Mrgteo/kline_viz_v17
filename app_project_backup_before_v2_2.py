...
CONFIG = {
    ...
    # A4 标准首板形态
    'penalty_a4_first_no_approx': 15,    # 标准档首板形态不近似扣分
    ...
    'case_library_cache': './stock_cache/case_library_break_v21.pkl',
    'worker_count': max(1, cpu_count() - 1),
}

# 把 cache_dir / case_library_cache 中的相对路径锚定到本文件所在目录，
# 避免 Streamlit / 任意 CWD 启动时把 './stock_cache/' 解析到错误位置。
...
SUBDIVISION_V2_INFO = {
    ...
    '震荡弱势长下影': {'score': -0.2, 'group': '宽幅震荡', 'emotion_class': '震荡'},
    # 窄幅震荡（2种）
    ...
    # 跌停类（5种，不变）
    '跌停': {'score': -2.0, 'group': '普通跌停类', 'emotion_class': '弱势'},
...
def match_kline_v2(t_sub, c_sub, t_pct=None, c_pct=None):
    ...
    可选参数 t_pct/c_pct（收盘涨跌幅）：用于判断假阴线/假阳线
    - 假阴线：被归为阴线类(小阴/中阴/大阴)但收盘涨幅>0
    - 假阳线：被归为阳线类(小阳/中阳/大阳)但收盘涨幅<0
    - 当双方同名但一真一假时，降级为'近似'
    """
    if t_sub == c_sub:
        # 同名情况下，检查阴线/阳线类的真假
        if t_pct is not None and c_pct is not None:
            ...
            if group == '阴线类':
                # 阴线类正常应pct<0；pct>0即为假阴线
                t_fake = t_pct > 0
                c_fake = c_pct > 0
                if t_fake != c_fake:
                    return '近似'
            elif group == '阳线类':
                # 阳线类正常应pct>0；pct<0即为假阳线
                t_fake = t_pct < 0
                c_fake = c_pct < 0
                if t_fake != c_fake:
                    return '近似'
    ...
    t_emo = t_info['emotion_class']
    c_emo = c_info['emotion_class']

    # 同近似组
    if t_group == c_group:
        if abs(t_score - c_score) <= 0.1:
            return '近似'
        else:
            return '情绪'  # 同组但分值差>0.1

    # 不同近似组，看情绪类
    if t_emo == c_emo:
        return '情绪'

    # 中性vs其他特殊处理：中性可以和任何匹配为情绪？
    # 按当前规则：不同情绪类直接不匹配
...
def build_all_break_cases(all_daily_data, stock_info_df):
    ...
    ac = []
    if wc <= 1:
        # 单进程直跑：Windows + 动态加载场景下，spawn 子进程无法 import
        # matcher_app_v17 → ModuleNotFoundError → pool.map 永远阻塞。
        p = 0
        for a in ta:
            p += 1
...
def calc_a_score(tc, cc):
    ...
    if ro_gap == 1:
        ...
        p += v
        d.append(f"最大涨幅跨1档({tc['max_rise_category']}vs{cc['max_rise_category']})-{v}")
    elif ro_gap == 0:
        t_mr_val = tc['max_rise']
        c_mr_val = cc['max_rise']
        max_val = max(abs(t_mr_val), abs(c_mr_val), 0.01)
        rel_diff = abs(t_mr_val - c_mr_val) / max_val
        if rel_diff <= 0.20:
...
def calc_b_score(tc, cc):
    ...
    if hr_gap_b == 1:
        ...
        p += v
        d.append(f"回撤比跨1档({tc['height_retracement_category']}vs{cc['height_retracement_category']})-{v}")
    elif hr_gap_b == 0:
        t_hr_v = tc['height_retracement']
        c_hr_v = cc['height_retracement']
        max_val_b = max(abs(t_hr_v), abs(c_hr_v), 0.01)
        rel_diff_b = abs(t_hr_v - c_hr_v) / max_val_b
        if rel_diff_b <= 0.20:
    ...
    # B10b 紧凑档断板天数精细匹配（一致+5，不一致-10）
    scope_b = tc['research_scope']
    if scope_b == '紧凑':
        if tc['break_actual_days'] == cc['break_actual_days']:
            v = CONFIG['bonus_b10b_match']
            b += v
            d.append(f"[紧凑]断板天数一致({tc['break_actual_days']}天)+{v}")
        else:
            v = CONFIG['penalty_b10b_mismatch']
            p += v
            d.append(f"[紧凑]断板天数不一致({tc['break_actual_days']}天vs{cc['break_actual_days']}天)-{v}")

    # B10c 紧凑档距D1精细匹配（一致+5，不一致-10）
    if scope_b == '紧凑':
        if tc['cut_to_d1_days'] == cc['cut_to_d1_days']:
            v = CONFIG['bonus_b10c_match']
            b += v
            d.append(f"[紧凑]距D1一致({tc['cut_to_d1_days']}天)+{v}")
        else:
            v = CONFIG['penalty_b10c_mismatch']
            p += v
            d.append(f"[紧凑]距D1不一致({tc['cut_to_d1_days']}天vs{cc['cut_to_d1_days']}天)-{v}")
...
def calc_e_score(tc, cc):
    ...
    if op_gap == 1:
        ...
        rel_diff = abs(t_op - c_op) / max_val
        if rel_diff <= 0.20:
